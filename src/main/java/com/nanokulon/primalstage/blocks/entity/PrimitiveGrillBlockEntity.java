...
public class PrimitiveGrillBlockEntity extends BlockEntity {
    ...
    private final RecipeManager.CachedCheck<Container, GrillRecipe> matchGetter = RecipeManager.createCheck(ModRecipes.GRILL.get());
    private final int[] cookingTimes = new int[4];
    private final int[] cookingTotalTimes = new int[4];

    public PrimitiveGrillBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.PRIMITIVE_GRILL_BLOCK_ENTITY.get(), pos, state);
    }
    ...
    public static void tick(Level world, BlockPos pos, BlockState state, PrimitiveGrillBlockEntity blockEntity) {
        ...
        if (bl) {
            PrimitiveGrillBlockEntity.setChanged(world, pos, state);
        }
    }
