...
public class PrimitiveGrillBlockEntityRenderer implements BlockEntityRenderer<PrimitiveGrillBlockEntity> {
    ...
    public PrimitiveGrillBlockEntityRenderer(BlockEntityRendererProvider.Context ctx) {
        this.itemRenderer = ctx.getItemRenderer();
    }

    @Override
    public void render(PrimitiveGrillBlockEntity grillBlockEntity, float f, PoseStack matrixStack, MultiBufferSource vertexConsumerProvider, int i, int j) {
        Direction direction = Direction.NORTH;
        NonNullList<ItemStack> defaultedList = grillBlockEntity.getItemsBeingCooked();
        int k = (int)grillBlockEntity.getBlockPos().asLong();
        for (int l = 0; l < defaultedList.size(); ++l) {
            ...
            this.itemRenderer.renderStatic(itemStack, ItemDisplayContext.FIXED, i, j, matrixStack,vertexConsumerProvider, grillBlockEntity.getLevel(), k + l);
            matrixStack.popPose();
        }
    }
}
